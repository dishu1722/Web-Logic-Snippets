...
$(document).on('click', '#service-submit-cart-btn', function(e) {
    ...
    var notesText = $('#service-dynamic-interaction-section textarea:visible').val() || '';
    var fileInput = document.getElementById('rx_upload');

    // Frontend Guardrail Check
    if ($('.rx-layout').is(':visible')) {
        if (!fileInput || fileInput.files.length === 0) {
            alert('Please upload your prescription before booking this service.');
            resetBtn();
            return; 
        }
    }
    ...
    // Explicitly append security nonce validation to avoid silent fails
    formData.append('security', '<?php echo wp_create_nonce("care-service-cart-nonce"); ?>');
    ...
    $.ajax({
        ...
        success: function(response) {
            // CRITICAL FIX: If response.success is false, reset the button and alert the error!
            if (response.success) {
                $(document.body).trigger('wc_fragment_refresh');
                $('.sticky-qty-selector').hide();

                var actionHtml = `
                    <div class="sticky-post-add-actions" style="display:flex; gap:10px; width:100%; flex-wrap:wrap;">
                        <button type="button" class="btn-continue-essentials" style="flex:1; min-width:220px; background:#fff; color:#243f2f; border:2px solid #243f2f; padding:12px 12px; border-radius:10px; font-weight:700; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:8px; text-transform:uppercase; transition:all .2s ease; font-size: 16px;">
                            🛒 Continue Shopping
                        </button>
                        <a href="${response.data.redirect_url}" class="btn-go-checkout" style="flex:1; min-width:220px; background:#243f2f; color:#fff; border:2px solid #243f2f; padding:12px 12px; border-radius:10px; font-weight:700; text-decoration:none; display:flex; align-items:center; justify-content:center; text-transform:uppercase; transition:all .2s ease; font-size: 16px;">
                            Checkout →
                        </a>
                    </div>
                `;
                ...
                // This unfreezes the button if the server rejected it
                alert(response.data || 'Could not add service to cart. Check server parameters.');
                resetBtn();
            }
        },
        error: function(xhr, status, error) {
            console.error('AJAX Error Response:', xhr.responseText);
            alert('Server error processing this checkout loop. Check browser console.');
            resetBtn();
        }
    });
...
#servicecare-trust-divider {
    display: none !important;
}
.is-care-service .c-product__col-2 .price, .is-care-service square-placement,  .is-care-service .price-subtext {
    text-align: center;
}
.whats-included-header h2, .essentials-section h2 {
    font-size: 25px !important;
}
#serviceopenFullList, .essentials-section h2 + a {
...
$is_rx = (
    ...
    <div id="servicewhats-included-section" class="whats-included-wrapper" style="display: none; padding-left: 30px; padding-right: 30px;">
        <div class="whats-included-header">
            <h2 style="margin: 0; font-weight: 700; text-align: left; font-size: 30px; line-height: normal;">What's included <span style="width: 25px; display: inline-block;">💚</span></h2>
            <a href="javascript:void(0);" id="serviceopenFullList" class="view-full-desc-trigger" style="color: #06aa64; font-size: 14px; font-weight: 700; cursor: pointer; letter-spacing: 0; text-align: right;">View more &rarr; </a>
        </div>
...
function service_essentials_system_v3() {
  ...
  <div class="essentials-section" style="background-color: #fbfbfb;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
            <h2 style="margin: 0; font-weight: 700; text-align: left;">Add essentials <span style="width: 25px; display: inline-block;">💚</span></h2>
        ...
        <p style="margin-top: 0; margin-bottom: 15px;">Include everyday items to suppport your loved one.</p>
...
jQuery(document).ready(function($) {
    ...
    // Continue Shopping button -> Open Essentials Modal
    $(document).on('click', '.btn-continue-essentials', function(e) {
        e.preventDefault();

        $('#serviceessentialsModal')
            .css('display', 'flex')
            .hide()
            .fadeIn(200);
    ...
    <div id="service-dynamic-interaction-section" class="service-interaction-wrapper" style="display: none;">
        <?php if ($is_rx) : ?>
            <div class="interaction-card rx-layout">
                <h3 class="interaction-title" style="text-align: left;">Prescription & Notes <span style="color: red;">*</span> <span style="width: 30px; display: inline-block;">💚</span></h3>
                <div class="interaction-grid">
                    <div class="upload-box" onclick="document.getElementById('rx_upload').click();">
                        ...
                        <div class="upload-content">
                            ...
                            <strong>Upload Prescription / Script <span style="color: red;">*</span></strong>
        ...
        <?php else : ?>
            <div class="interaction-card standard-layout">
                <h3 class="interaction-title" style="text-align: left;">Additional notes <span style="width: 30px; display: inline-block;">💚</span></h3>
        ...
        <?php endif; ?>
<div class="service-action-footer" style="margin-top: 25px; padding-top: 20px; border-top: 1px solid #f0f0f0; display: flex; justify-content: center; align-items: center; gap: 20px; max-width: 640px; margin: 8px auto;">
...
function handle_care_service_cart_upload() {
    ...
    // Security Nonce Verification Validation
    if ( ! isset($_POST['security']) || ! wp_verify_nonce($_POST['security'], 'care-service-cart-nonce') ) {
        wp_send_json_error('Security validation check failed.');
    }
    ...
    // SMART BACKEND FILE ENFORCEMENT
    // We fetch the product title to see if it's actually a prescription service layout
    $product_title = get_the_title($product_id);
    $is_prescription_service = (strpos(strtolower($product_title), 'prescription') !== false);

    if ( $is_prescription_service ) {
        // Strict file requirement ONLY for prescription services
        if ( empty($_FILES['rx_file']) || $_FILES['rx_file']['error'] !== UPLOAD_ERR_OK ) {
            wp_send_json_error('Prescription file upload is required to complete this booking.');
        }
    }

    // Only process file uploads if a file was actually sent (avoids errors on standard notes)
    if ( !empty($_FILES['rx_file']) && $_FILES['rx_file']['error'] === UPLOAD_ERR_OK ) {
    ...
    // Add item to cart session with custom data payload
...
/**
 * 1. Read custom data from AJAX payload and append it to the WooCommerce Cart Item Session
 */
add_filter( 'woocommerce_add_cart_item_data', 'save_prescription_fields_to_cart_session', 10, 3 );
function save_prescription_fields_to_cart_session( $cart_item_data, $product_id, $quantity ) {
    
    // Catch notes if passed through $_POST
    if ( isset( $_POST['notes'] ) && ! empty( $_POST['notes'] ) ) {
        $cart_item_data['care_service_notes'] = sanitize_textarea_field( $_POST['notes'] );
    }

    // Catch uploaded file properties passed via your handle_care_service_cart_upload handler
    if ( isset( $_POST['rx_file_url'] ) ) {
        $cart_item_data['care_service_rx_url']  = esc_url_raw( $_POST['rx_file_url'] );
        $cart_item_data['care_service_rx_name'] = sanitize_text_field( $_POST['rx_file_name'] );
    }
...
/**
 * 2. Display the Custom Meta inside the WordPress Admin Order Details screen table
 */
add_action( 'woocommerce_before_order_itemmeta', 'render_prescription_meta_in_admin_order_table', 10, 3 );
function render_prescription_meta_in_admin_order_table( $item_id, $item, $product ) {
    // Check if the order line item has our custom saved keys
    $notes   = $item->get_meta( 'Customer Notes' );
    $rx_url  = $item->get_meta( 'Prescription URL' );
    $rx_name = $item->get_meta( 'Prescription Name' );

    // Fallback checks for legacy key syntax mappings
    if ( empty( $notes ) )   $notes   = $item->get_meta( '_care_service_notes' );
    if ( empty( $rx_url ) )  $rx_url  = $item->get_meta( '_care_service_rx_url' );
    if ( empty( $rx_name ) ) $rx_name = $item->get_meta( '_care_service_rx_name' );

    // Render out cleanly inside the order dashboard table line row
    if ( ! empty( $notes ) ) {
        echo '<p style="margin: 5px 0; font-size: 13px;"><strong>Customer Notes:</strong> ' . esc_html( $notes ) . '</p>';
    }
    if ( ! empty( $rx_url ) ) {
        $display_title = ! empty( $rx_name ) ? $rx_name : 'View Attached Document';
        echo '<p style="margin: 5px 0; font-size: 13px;"><strong>Prescription Script:</strong> <a href="' . esc_url( $rx_url ) . '" target="_blank" style="color: #007cba; text-decoration: underline; font-weight: 600;">📄 ' . esc_html( $display_title ) . '</a></p>';
    }
}

/**
 * 3. Persist session keys directly into permanent Order Meta records on checkout submission
 */
add_action( 'woocommerce_checkout_create_order_line_item', 'persist_cart_session_to_order_meta_records', 10, 4 );
function persist_cart_session_to_order_meta_records( $item, $cart_item_key, $values, $order ) {
    if ( isset( $values['care_service_notes'] ) ) {
        $item->update_meta_data( 'Customer Notes', $values['care_service_notes'] );
        $item->update_meta_data( '_care_service_notes', $values['care_service_notes'] );
    }
    if ( isset( $values['care_service_rx_url'] ) ) {
        $item->update_meta_data( 'Prescription URL', $values['care_service_rx_url'] );
        $item->update_meta_data( '_care_service_rx_url', $values['care_service_rx_url'] );
        $item->update_meta_data( 'Prescription Name', $values['care_service_rx_name'] );
        $item->update_meta_data( '_care_service_rx_name', $values['care_service_rx_name'] );
    }
}